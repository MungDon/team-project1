...
<TABLE>
	...
	<TR>
		...
		<%-- 수정) 주문페이지에서도 수량 조절 버튼 구현할지 결정하기 --%>
...
<%
	for (ProductDTO productDTO : list) {
		for (ImgDTO orderImg : productDTO.getImages()) {
...
%>
	<TR>
		<TD width="50" height="65" style="border-right:hidden;">
			<A href="#"><img src="../upload/<%= orderImg.getImg_name() %>" width="40" height="40" onclick="window.location='../product/productContent.jsp?product_num=<%= product_num %>&category_num=<%= category_num %>'"	/></A>
		</TD>
		<TD align="left" colspan="1" style="border-left:hidden; border-right:hidden;">
...
<%
		...
		String fmTotalPrice = formatter.format(count * dto.getPrice());
		String fmDeliveryPrice = formatter.format(dto.getDelivery_price());
		String fmFinalPrice = formatter.format((count * dto.getPrice()) + dto.getDelivery_price());
%>
<br /><br />
<TABLE style="border: 2px solid #BDBDBD;" cellspacing="0" cellpadding="0" height="150">
	<TR>
		...
		<TD style="text-align:center; border-left:hidden; border-right:hidden;">
			<div style="text-align:right;">총 <B><%= count  %></B> 개의 상품금액</div>
		...
		<TD style="border-left:hidden; border-right:hidden;">
			...
			<div style="text-align:right;"><font color="#6799FF"><B><%= fmDeliveryPrice %>원</B></font></div>
		</TD>
		<TD style="border-left:hidden; border-right:hidden; width:50;">
...
%>
	<input type="hidden" name="delivery_name" value="<%= deliveryDto.getDelivery_name() %>">
...
<TABLE style="border: 1px solid gray; border-left:hidden; border-right:hidden; border-top:hidden; border-bottom:1px solid #EAEAEA;" cellspacing="0" cellpadding="0">
	...
	<TR>
		...
		<TD align="left" style="border-bottom:1px solid #EAEAEA;">
			...
			<select name="delivery_num" style="width:280;">
<%
	for (DeliveryDTO deliveryDto : deliveryList) {
%>
	...
	<TR>
		...
		<TD align="left" style="border-bottom:1px solid #EAEAEA;">
			<%if(memberDto.getPhone()!=null){ %>
				<input type="text" name="o_phone" id="o_phone" value="<%= memberDto.getPhone() %>"	/>
			<%}else{ %>
				<input type="text" name="o_phone" id="o_phone"/>
				<input type="hidden" name="o_phone" value="none"/>
			<%} %>
...
<TABLE style="border: 1px solid gray; border-left:hidden; border-right:hidden; border-top:hidden; border-bottom:1px solid #EAEAEA;" cellspacing="0" cellpadding="0">
	...
	<TR>
		...
		<TD align="left" style="border-bottom:1px solid #EAEAEA;">	<%-- 수정) 직접 입력 일 때 값 "" (공백) if 걸어서 서밋 못하게 + alert 창 --%>
			<input type="radio" name="default_address" id="defualt" value="2" onclick="getDelivery(event)"			/><font size="2">기본 배송지</font>			<%-- value 보내야함 --%>
			&nbsp;&nbsp;&nbsp;
	...
	<TR>
		...
		<TD align="left" style="border-bottom:1px solid #EAEAEA;">
				...
				<input type="hidden" id="h_phone" name="phone" value="n"	/>
		</TD>
	</TR>
	<TR>
		<TD width="1%" style="border-bottom:1px solid #EAEAEA;">
...
<TR>
		...
		<TD align="left">
			...
			<input type="checkbox" name="memberUpdate" id="checkbox" value="1"		/>				<%-- /member/memberUpdate.jsp / pro 에서 파라미터 받아야함 -완- --%>
...
<TABLE style="border: 1px solid gray; border-left:hidden; border-right:hidden; border-top:hidden; border-bottom:1px solid #EAEAEA;" cellspacing="0" cellpadding="0">
	...
	<TR>
		...
		<TD align="left" style="border-bottom:1px solid #EAEAEA;">
			<font size="4"><B><%= fmTotalPrice %>원</B></font>
		</TD>
	</TR>
	<TR>
		<TD width="1%" style="border-bottom:1px solid #EAEAEA;">
		...
		<TD align="left" style="border-bottom:1px solid #EAEAEA;">
			<font size="2"><%= fmDeliveryPrice %>원</font>
		</TD>
	</TR>
	<TR>
		<TD width="1%">
...
<%-- 
	...
	document.getElementById("btn_submit").addEventListener("click", function(event) {
        ...
        var errorMessage = "";
        if (!orders_nameValue || !m_cellphoneValue || !emailValue || !receiver_nameValue || !address1Value || !address2Value || !address3Value || !cellphoneValue) {
...
<script>
	...
	document.getElementById('email').addEventListener('input', function() {
   	 	let phone_value = document.getElementById('email').value;
    	document.getElementById('h_email').value = phone_value;
});
...
//		--------------------
	...
	if (selectedValue === "0") {
		if(<%=memberDto.getPhone()%>==null){
			...
			if (selectedValue === "0") {
				document.getElementById().value = "<%=memberDto.getPhone()%>";	
			}
		}
	}
}
document.getElementById("btn_submit").addEventListener("click", function(event) {
	...
	if (errorMessage) {					// 값이 비어있지 않다면
		...
		document.getElementById("orderForm").submit();		// 모든 필수 정보가 제대로 입력되었으므로 폼을 제출함
	}
});
</script>
